...
### 병렬 트윈에서 `chain()`은 가장 느린 것까지 기다린다

- `set_parallel()` 뒤의 `chain()`은 **앞의 모든** 트위너가 끝난 뒤에 다음을 시작한다
- 그래서 짧은 동작에 이어붙이려 해도 같은 트윈 안의 긴 동작이 끝날 때까지 멈춰 있는다
- 커졌다 돌아오는 팝에 색 변화를 같이 묶으면, 커진 채로 멈췄다가 뒤늦게 돌아온다
- 길이가 다른 동작은 트윈을 따로 만든다. 한 트윈에 묶는 건 순서가 실제로 필요할 때만
- 적용 사례: 습관 칸의 팝과 색 채우기
...
- ⚠️ `PopupPanel`은 자식 Control의 `position`과 `size`를 매 배치마다 덮어쓴다(`scene/gui/popup.cpp`)
- 그래서 안쪽에서도 **이동은 못 쓴다.** `scale`과 `modulate`는 안 건드리므로 그 둘로 연출한다
- 래퍼를 끼우는 방법도 안 된다. 창 크기가 내용의 최소 크기에서 나오는데 래퍼는 그것을 위로 안 올린다
