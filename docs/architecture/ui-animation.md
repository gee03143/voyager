...
### 앵커된 컨트롤에 `position`을 쓰면 그때의 크기가 오프셋으로 굳는다

- 앵커가 잡힌 컨트롤에서 `position`은 오프셋으로 환산돼 저장된다
- 그 환산은 **그 순간의 크기**를 기준으로 한다. 크기가 부모와 어긋나 있으면 그 차이가 그대로 박힌다
- 첫 레이아웃 전에는 래퍼가 0이고 패널은 자기 최소 크기다
  - 이때 이동을 걸면 `오프셋 = 최소 크기`가 굳는다
  - 이후 래퍼가 커져도 `크기 = 래퍼 + 최소 크기`가 되어 오른쪽·아래가 래퍼 밖으로 밀려난다
- 클리핑이 걸려 있으면 밀려난 만큼이 조용히 잘린다. 화면에는 "왜인지 잘림"으로만 보인다
- 안쪽에 `ScrollContainer`가 있으면 증상이 한 겹 더 꼬인다
  - 스크롤 컨테이너는 잘린 높이까지 자기 것으로 알고 있어 스크롤바를 띄우지 않는다
  - 스크롤이 안 먹는 것처럼 보이지만 실제로는 스크롤할 이유가 없다고 판단한 것이다
- 이동을 걸기 전과 끝난 뒤에 `set_anchors_and_offsets_preset()`으로 오프셋을 다시 0으로 돌린다
- 적용 사례: 탭·화면이 래퍼보다 최소 크기만큼 커져 오른쪽이 잘리고 목록 스크롤이 죽던 문제
...
### 스타일박스를 비울 때 여백을 잃으면 버튼이 줄어든다

- 버튼 배경을 직접 그리려면 원래 스타일박스를 비워야 하는데, 빈 것으로 덮으면 여백까지 사라진다
- `Button`은 **현재 상태의** 스타일박스로 최소 크기를 잰다
  - 근거: `scene/gui/button.cpp`의 `get_minimum_size_for_text_and_icon` 마지막 줄
  - 기본 테마가 `align_to_largest_stylebox`를 0으로 두고 이 프로젝트는 안 덮는다
- 그래서 `pressed`만 비우면 **선택된 버튼만** 그 여백만큼 작아진다
- `StyleBoxEmpty`에도 `content_margin_*`이 있으므로 원래 여백을 옮겨 담는다
- 적용 사례: 탭 선택 채움
...
### `_draw()`로 그리는 위젯은 시간 하나를 트윈한다

- 막대 차트처럼 요소마다 노드가 없는 위젯은 트윈을 걸 속성이 없다
- 대신 경과 시간을 담은 변수 하나를 `tween_method`로 굴리고, 매 프레임 `queue_redraw()`한다
- `_draw()`가 그 값을 읽어 요소마다 자기 진행률을 계산한다. 요소별 시작 지연도 여기서 뺄셈으로 낸다
- 요소마다 목표치가 다르고 속도가 고정이면 **끝나는 시각이 제각각**이다
  - 트윈 길이는 가장 늦게 끝나는 요소를 따로 계산해서 잡는다
  - 합계나 최댓값 하나로 잡으면 다 끝난 뒤 빈 시간이 남는다
- 값을 세팅하는 경로와 연출을 시작하는 경로를 나눈다. 연출 없이 즉시 반영해야 하는 호출이 반드시 있다
- 연출 중인지 여부를 플래그로 들고, 꺼져 있으면 `_draw()`는 최종 상태를 그린다
- 적용 사례: 그래프 막대 차오름
