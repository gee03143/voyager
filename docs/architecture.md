...
## Week 4 구현 결과 (T1~⑤ 완료, 2026-06-18 · 다음 = ⑥ 발견)
계획 대비 ⑤에서 **항해 모델이 재설계**됨 — 아래가 확정 사실(위 "데이터/빌드순서" bullet은 초기 계획안):
...
- **어휘 해금 = 복합 모델**(사용자 결정): ⑴ 자체 발견(섬 발견 시 테마 단어 — Week 5) ⑵ 집중 마일스톤(`total_focus_seconds` 임계 — Week 6) ⑶ 받은 편지에서 **확률적 구절 보관**(수신 시 — Week 6). ⑶은 어휘가 항해자 사이로 **전파** → 연결성이 매체 자체에 체화.
...
- **슬라이스(주 분산, 각 F6, 범위축소 아님)**:
  - **⑥ Week 4 = 받기 루프(수신·수집)**: 템플릿+슬롯 콘텐츠 + 시드 편지 풀 + 토큰 렌더러 / 집중 항해 중 병 **앰비언트 발견**(비방해 토스트, 열람은 휴식 보상) / 양피지 열람 / 선반 수집(`Save.letters`). 컴포저·해금 불필요(수신 전용) → MVP-safe. (※ 원안 ⑥의 "섬 스크롤 등장"은 Week 5 지도/도감으로 이동.)
  - **Week 5 = 지도+섬 도감 + 자체발견 해금**: 섬 원경 스크롤 등장→`discovered_islands`→지도·도감, 섬 발견 = 테마 어휘 해금(`lexicon` 도입). 병 선반도 도감 패턴 합류.
  - **Week 6 = 작성(나도 띄우기) + 일지 + 집중·수신 해금**: 템플릿+슬롯 컴포저(해금 어휘 조합), 세션 종료 회고 = 일지, `sent_letters` 로컬 저장.
  - **Phase 3(서버) = 진짜 교환**: 실송수신·좋아요·신고/차단·모더레이션 파이프라인·크로스랭귀지 라이브.
- **구현 분리(노드/클래스)**: 발견 메커니즘 = 범용 **`Discovery`**(Node, `interval` 거리 임계마다 `discovered` 발신, **콘텐츠 무지** — Week 5 섬도 재사용). **`Mailbox`**(Node, 편지 도메인: 트레이 `_pending` + 토스트·인디케이터·열람 라우팅). **`LetterView`**(양피지 렌더, `Save` 모름 — `kept` 의도만 발신). **`LetterContent`**(템플릿·단어·시드·`render`, 순수 static). **`LetterArchive`**(보관 영속 모델). 거리는 집중 중에만 증가 → 발견도 집중에만(자동 게이트), 열람은 비집중(항구)에서만.
  - **진입 UI(다이제틱)**: 여가(편지/선반)는 도크 버튼이 아니라 **항구 월드 오브젝트** 클릭으로 진입. 정박(비집중) 시 화면 밖에서 슬라이드-인, 항해 시 슬라이드-아웃(`Shelf` Node2D, `Clock.is_focusing` 폴링). "받은 편지 N" 배지는 오브젝트 위. 완전한 항구(배경 아트·입출항 전환·여러 오브젝트)는 Week 5. 도크는 생산성(시계/할일/습관)만 유지.
  - **열린 질문(보류, 2026-06-19)**: 여가 위젯이 열린 채 휴식 종료→집중 자동 시작 시 처리. 후보 = 집중 진입 시 여가 UI 자동 닫기 + 읽던 편지 트레이 복원(`Clock.focusing_changed` 신호 신설), 또는 휴식→집중 자동시작 자체를 막기. **나중에 재논의.**
