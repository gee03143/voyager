...
- **어휘 해금**: 활동 어휘 = *그 활동을 함으로써 영구 해금*(누적, 안 줄어듦 — 내 실천의 지문). 표현/뉘앙스 어휘 = 집중 마일스톤 + 받은 편지에서 **확률적 구절 보관**(→ 어휘가 항해자 사이로 **전파**, 연결성이 매체에 체화). 진실성은 어휘 잠금이 아니라 그날 로그 그라운딩이 담당. 상세 플랜·슬라이스 = roadmap.
...
- **구현 분리(노드/클래스)**: 발견 메커니즘 = 범용 **`Discovery`**(Node, `interval` 거리 임계마다 `discovered` 발신, **콘텐츠 무지** = 범용 발견 프리미티브). **`Mailbox`**(Node, 편지 도메인: 트레이 `_pending` + 토스트·인디케이터·열람 라우팅). **`LetterView`**(양피지 렌더, `Save` 모름 — `kept` 의도만 발신). **`LetterContent`**(템플릿·단어·시드·`render`, 순수 static). **`LetterArchive`**(보관 영속 모델). 거리는 집중 중에만 증가 → 발견도 집중에만(자동 게이트), 열람은 비집중(항구)에서만.
