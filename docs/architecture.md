...
- **데이터**: `Save.voyage = {total_focus_seconds, distance, discovered_islands:[]}` + **version 4** 마이그레이션.
  - 누적 = **포모 `focus_finished`(집중 단계) + 일반 타이머 완료** 시 (사용자 결정). 메커니즘은 `Save` 모름 유지 → `Save` 아는 뷰/컨트롤러가 배선. **즉시 저장**(사건=커밋, save-on-start와 같은 결).
  - 섬 = 지도·도감이 **참조**하는 엔티티 → **안정 ID**(습관의 `randi` 패턴 재사용). 발견 기록은 ID 목록.
- **빌드 순서(각 단계 F6 검증, 범위 축소 아님)**: ⓪데이터 → ①월드 루트 → ②도크+패널 호스트 → ③항해 누적 배선(해리 배지) → ④상시 타이머 HUD → ⑤배 반응(해리 반영) → ⑥발견.
