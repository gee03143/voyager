...
- **데이터 모델**(안정 ID·JSON·version 일관, 서버 대비): 병 = `{id(randi), template_id, slots:[id], author("이름 모를 항해자"→실유저id로 스왑), found_ts, like_count(예약), report(예약)}`. `Save.collected_letters`(선반=발견 도감 첫 인스턴스), `Save.lexicon{template_ids, word_ids}`(해금분), `Save.sent_letters`(W6 로컬). 템플릿·단어 = 번역 가능 콘텐츠 리소스.
- **슬라이스(주 분산, 각 F6, 범위축소 아님)**:
  - **⑥ Week 4 = 받기 루프(수신·수집)**: 템플릿+슬롯 콘텐츠 + 시드 편지 풀 + 토큰 렌더러 / 집중 항해 중 병 **앰비언트 발견**(비방해 토스트, 열람은 휴식 보상) / 양피지 열람 / 선반 수집(`collected_letters`). 컴포저·해금 불필요(수신 전용) → MVP-safe. (※ 원안 ⑥의 "섬 스크롤 등장"은 Week 5 지도/도감으로 이동.)
