...
README의 3축 중 **활력·함께 있음**, 할 일/습관 완료 반응, 대화 라우팅이 구현됨. **기억 축은 미구현** — 선택 기준은 `companion-persona.md`에서 정해졌다(헤이즐은 검색하지 않고 묻어둔 걸 우연히 파낸다 = 무작위 선택이 캐릭터의 성질). 남은 조건은 "아물 시간이 지난 것만" 판정할 기준 일수와 3단계 게이트.

⚠️ **캐릭터 규격(종·이름·로어·호칭·화법 규격·금지 규칙·친밀도 단계)은 `companion-persona.md`가 갖는다.** 이 절은 코드 구조만 다룬다 — 대사를 추가하거나 반응을 늘릴 때는 그쪽 문서의 §0 빠른 판정을 먼저 통과시킬 것. **친밀도(만난 날 수)·호칭·생일은 아직 데이터 모델이 없다.**
